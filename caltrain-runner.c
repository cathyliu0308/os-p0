	...
	while (passengers_left > 0) {
		...
		while (threads_reaped < threads_to_reap) {
			...
			if (threads_completed > 0) {
                ...
                printf("runner threads_completed 0 %d threads_reaped %d\n",threads_completed,threads_reaped);
		...
		if (!load_train_returned) {
			...
			exit(1);
                ...
                printf("runner threads_completed 1 %d threads_reaped %d\n",threads_completed,threads_reaped);
		while (threads_completed > 0) {
			threads_reaped++;
			__sync_sub_and_fetch(&threads_completed, 1);
		}
                printf("runner threads_completed 2 %d threads_reaped %d\n",threads_completed,threads_reaped);
